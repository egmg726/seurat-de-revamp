...
# In this example, we’ll use the BlueprintEncodeData as the reference data as 
# it contains blood & immune lineages
ref.set <- celldex::BlueprintEncodeData()

#The main labels 
unique(ref.set$label.main)

#Using SingleR to label our cells with the above reference dataset 
sce.ifnb <- logNormCounts(sce.ifnb )
pred.cnts <- SingleR::SingleR(test = sce.ifnb  , ref = ref.set, labels = ref.set$label.main)

lbls.keep <- table(pred.cnts$labels)>10
# Add SingleR labels to Seurat metadata
ifnb.filtered$SingleR.labels <- sce.ifnb$SingleR.labels
