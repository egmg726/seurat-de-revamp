...
```
Barcode rank plot 
- Shows the distribution of UMI totals among kept cells
```{r}
# Split Seurat object by treatment group
ifnb.list <- SplitObject(ifnb, split.by = "orig.ident")

# Run barcodeRanks() on each treatment group
br.list <- lapply(names(ifnb.list), function(grp) {
  counts_mat <- as.matrix(GetAssayData(ifnb.list[[grp]], slot = "counts"))
  br <- barcodeRanks(counts_mat)
  ...
  attr(df, "meta") <- metadata(br) # save knee/inflection
  df
})

# Combine into one dataframe
df.all <- do.call(rbind, br.list)

# Extract metadata (knee & inflection per group)
# meta.df <- do.call(rbind, lapply(br.list, function(d) {
#   data.frame(
#     treatment = unique(d$treatment),
#     knee = attr(d, "meta")$knee,
#     inflection = attr(d, "meta")$inflection
#   )
# }))

# Plot faceted barcode rank plots
ggplot(df.all, aes(x = rank, y = total)) +
  geom_point(size = 0.4, alpha = 0.6) +
  # geom_hline(data = meta.df, aes(yintercept = knee), 
  #            color = "dodgerblue", linetype = "dashed") +
  # geom_hline(data = meta.df, aes(yintercept = inflection), 
  ...
  labs(
    x = "Barcode Rank",
    y = "Total UMI Count",
    title = "Barcode Rank Plots (Post-Filtering)"
  ) +
...
top.hvg <- getTopHVGs(dec, prop = 0.1)

# Dimensionality reduction (PCA and t-SNE)
set.seed(101010011)
sce <- denoisePCA(sce, technical = dec, subset.row = top.hvg)
...
# Build SNN graph and assign cluster labels
# set.seed(101)
# snn.gr <- buildSNNGraph(sce, use.dimred = "PCA", k = 25)
...
Check 1 — BEFORE integration (after PCA / pre-Harmony/CCA) (Additional code)
...
::::::::::::::::::::::::::::::::::::: challenge 
Check 2 — AFTER integration (after umap.cca + clustering)
